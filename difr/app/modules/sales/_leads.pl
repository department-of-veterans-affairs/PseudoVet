...
sub insert{
  ...
  if($fail eq 'false'){
    ...
    values('0','$g->{alias}','$g->{lastname}','$g->{firstname}','$g->{middle}','$g->{suffix}','$g->{degree}',
           '$g->{jobtitle}','$g->{company}',
           '$g->{addressline1}','$g->{addressline2}','$g->{city}','$g->{stateprovince}','$g->{postalcode}',
           '$g->{country}','$g->{language}','$g->{homephone}','$g->{facilityid}','$g->{mailcode}',
           '$g->{workphone}','$g->{workext}','$g->{cellphone}','$g->{fax}','$g->{email}',
           '$g->{status}','$g->{comment}')";
   ...
   # compose email message payload
    ...
    my $emailcmd="/usr/sbin/sendmail -F'EtherFeat' -f'support\@etherfeat.com' -v will\@etherfeat.com < $g->{tempfiles}/$filename >> $g->{tempfiles}/mail.log";
    #print "$emailcmd\n";
    system("$emailcmd");
