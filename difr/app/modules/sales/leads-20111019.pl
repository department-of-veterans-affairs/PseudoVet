...
sub insert{
  ...
  if($fail eq 'false'){
    ...
    values('0','$g->{alias}','$g->{lastname}','$g->{firstname}','$g->{middle}','$g->{suffix}','$g->{degree}','$g->{jobtitle}','$g->{company}',
    '$g->{addressline1}','$g->{addressline2}','$g->{city}','$g->{stateprovince}','$g->{postalcode}','$g->{country}','$g->{language}',
    '$g->{homephone}','$g->{facilityid}','$g->{mailcode}','$g->{workphone}','$g->{workext}','$g->{cellphone}','$g->{fax}','$g->{email}',
    '$g->{status}','$g->{comment}')";
    ...
    $message.="Subject:EtherFeat :: New Lead Notification\n";
    $message.="<img src=\"http://www.etherfeat.com/logo.gif\" />";
    $message.="\n\n";
    $message.="<h3>New Sales Lead added to DIFR</h3>\n\n";
    $message.="<table cols=2>\n";
    ...
    $message.="<p>Lead added by: $g->{sys_username} on $g->{now}</p>\n";
    ...
    my $emailcmd="/usr/sbin/sendmail -F'EtherFeat' -f'support\@etherfeat.com' -v sales\@etherfeat.com < $g->{tempfiles}/$filename >> $g->{tempfiles}/mail.log";
    #print "$emailcmd\n";
    system("$emailcmd");
